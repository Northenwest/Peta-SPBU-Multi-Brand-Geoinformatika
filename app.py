...
[data-testid="stSidebar"] { background: #12151C; border-right: 1px solid #1E2230; transition: transform 0.35s cubic-bezier(.4,0,.2,1), opacity 0.35s ease; }
...
#sidebar-toggle-btn {
    ...
    width: 22px;
    height: 64px;
    background: linear-gradient(180deg, #A855F7 0%, #6B9FE4 100%);
    border: none;
    ...
    box-shadow: 3px 0 16px rgba(168,85,247,0.35);
    transition: width 0.2s ease, left 0.35s cubic-bezier(.4,0,.2,1), background 0.2s ease;
    padding: 0;
}
#sidebar-toggle-btn:hover {
    width: 28px;
    background: linear-gradient(180deg, #C084FC 0%, #93B9F5 100%);
    box-shadow: 4px 0 20px rgba(168,85,247,0.55);
}
...
/* When sidebar is OPEN — button sits at right edge of sidebar */
body[data-sidebar="open"] #sidebar-toggle-btn {
    left: var(--sidebar-width, 320px);
}
body[data-sidebar="open"] #sidebar-toggle-btn .toggle-arrow {
    transform: rotate(180deg);
}

/* When sidebar is CLOSED */
body[data-sidebar="closed"] #sidebar-toggle-btn {
    left: 0;
}

/* Sidebar collapsed state */
body[data-sidebar="closed"] [data-testid="stSidebar"] {
    transform: translateX(-100%);
    opacity: 0;
    pointer-events: none;
}
body[data-sidebar="closed"] [data-testid="stSidebarNav"] {
    display: none;
}

/* Smooth main content shift */
body[data-sidebar="closed"] section.main {
    margin-left: 0 !important;
    transition: margin-left 0.35s cubic-bezier(.4,0,.2,1);
}
body[data-sidebar="open"] section.main {
    transition: margin-left 0.35s cubic-bezier(.4,0,.2,1);
}
...
<!-- ── SIDEBAR TOGGLE BUTTON (injected into DOM) ── -->
<button id="sidebar-toggle-btn" data-tip="Sembunyikan Sidebar" onclick="toggleSidebar()" title="">
...
(function() {
    ...
    function applyState() {
        ...
        if (sidebarOpen) {
            ...
            btn.setAttribute('data-tip', 'Sembunyikan Sidebar');
        } else {
            document.body.setAttribute('data-sidebar', 'closed');
            btn.setAttribute('data-tip', 'Tampilkan Sidebar');
        }
    }
    ...
    function init() {
        applyState();
        // Re-measure sidebar width after Streamlit finishes rendering
        setTimeout(function() {
            if (sidebarOpen) applyState();
        }, 600);
    }
    ...
    // Re-attach on Streamlit re-renders (mutation observer on body)
    var observer = new MutationObserver(function(mutations) {
        var btn = document.getElementById('sidebar-toggle-btn');
        if (!btn || btn.getAttribute('data-init') === '1') return;
