...
with map_col:
    ...
    # Pin lokasi pengguna (dari klik peta)
    if user_loc_active:
        folium.Marker(
    ...
    # Proses klik pada peta (bukan pada popup/marker)
    if map_data and map_data.get("last_clicked"):
        clicked   = map_data["last_clicked"]
        new_lat   = clicked.get("lat")
        new_lng   = clicked.get("lng")
        if new_lat is not None and new_lng is not None:
            # Hanya update jika koordinat benar-benar berubah (hindari loop rerender)
