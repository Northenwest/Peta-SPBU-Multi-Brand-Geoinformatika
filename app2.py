...
/* Hide the collapse button Streamlit renders */
...
# Road color mapping by highway type
ROAD_COLORS = {
    "motorway":       "#00F5FF",
...
# Road types to skip rendering entirely
ROAD_SKIP_TYPES = {
...
with map_col:
    ...
    if map_data and map_data.get("last_clicked"):
        ...
        if new_lat is not None and new_lng is not None:
            if (new_lat != st.session_state.user_lat or new_lng != st.session_state.user_lng):
...
with ch4:
    ...
    for key, label in fuel_map_display.items():
        count = sum(1 for feat in raw_geojson["features"]
                    ...
                    and feat["properties"].get(key) == "yes"
                    and feat["properties"].get("brand", "") in selected_brands)
        if count > 0:
