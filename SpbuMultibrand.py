...
def load_and_process():
    try:
        with open("SPBUMB_REVISI.geojson", "r") as f:
            raw = json.load(f)
    except FileNotFoundError:
        # Fallback dummy jika data tidak ditemukan agar app tidak langsung crash saat inisialisasi awal
        st.error("File SPBUMB_REVISI.geojson tidak ditemukan!")
        return pd.DataFrame(), [], []

    point_features   = [f for f in raw["features"] if f["geometry"]["type"] == "Point"]
    line_features    = [f for f in raw["features"] if f["geometry"]["type"] == "LineString"]
    polygon_features = [f for f in raw["features"] if f["geometry"]["type"] == "Polygon"]
    ...
    return pd.DataFrame(records), line_features, polygon_features

df_all, road_features, polygon_features = load_and_process()
...
ROAD_COLORS = {
    "motorway":       "#00F5FF",
    "motorway_link":  "#00F5FF",
    "trunk":          "#00CFFF",
    "trunk_link":     "#00CFFF",
    "primary":        "#0096FF",
    "primary_link":   "#0096FF",
    "secondary":      "#0060CC",
    "secondary_link": "#0060CC",
}
...
with st.sidebar:
    ...
    st.markdown('<div class="filter-header">📍 Lokasi Saya (Klik pada Peta)</div>', unsafe_allow_html=True)

    if st.session_state.user_lat is not None:
        st.markdown(f"""
        <div style="background:rgba(168,85,247,.12);border:1px solid rgba(168,85,247,.3);
                    border-radius:10px;padding:10px 14px;margin-bottom:8px">
            <div style="font-size:10px;color:#A855F7;font-weight:700;text-transform:uppercase;
                        letter-spacing:.08em;margin-bottom:4px">📍 Lokasi Dipilih</div>
            <div style="font-size:12px;color:#C084FC;font-family:monospace;line-height:1.6">
                {st.session_state.user_lat:.6f}<br>{st.session_state.user_lng:.6f}
            </div>
        </div>
        """, unsafe_allow_html=True)
        if st.button("🗑 Hapus Lokasi", use_container_width=True):
            st.session_state.user_lat = None
            st.session_state.user_lng = None
            st.rerun()
    else:
        st.markdown("""
        <div style="background:rgba(255,209,102,.06);border:1px dashed rgba(255,209,102,.2);
                    border-radius:10px;padding:10px 14px;margin-bottom:8px">
            <div style="font-size:12px;color:#5A5F72;line-height:1.7">
                🖱 <b style="color:#8A8FA8">Klik di mana saja pada peta</b> untuk menentukan lokasi Anda dan mencari SPBU terdekat.
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    brands_available = sorted(df_all["brand"].unique()) if not df_all.empty else []
    ...
    st.markdown('<div class="filter-header">🗂 Layer Peta</div>', unsafe_allow_html=True)
    show_polygon  = st.checkbox("🏙 Batas Kota Bekasi", value=True)
    show_roads    = st.checkbox("🛣 Jaringan Jalan", value=True)
    ...
    🏙 Wilayah: Kota Bekasi<br>
    🔢 Total: {len(df_all)} lokasi SPBU<br>
    🛣 Jalan: {len(road_features)} segmen
    </div>
    """, unsafe_allow_html=True)
...
if user_loc_active and not df.empty:
    ...
    if not df.empty:
        df["distance_km"] = None
    df_within_radius  = pd.DataFrame()
    df_nearest        = pd.DataFrame()


# ── HEADER ────────────────────────────────────────────────────────────────────
st.markdown('<div class="hero-title">📍 Peta SPBU Multi-Brand Kota Bekasi</div>', unsafe_allow_html=True)
st.markdown('<div class="hero-sub">Dashboard Analisis Spasial Bahan Bakar Kota Bekasi · Jeremy</div>', unsafe_allow_html=True)
st.markdown("<br>", unsafe_allow_html=True)


# ── STAT CARDS ────────────────────────────────────────────────────────────────
total = len(df)
brand_counts = df["brand"].value_counts() if not df.empty else pd.Series()
...
metric_card(cols_cards[5], "total",     "🏧", int(df["atm"].sum()) if not df.empty else 0,             "Ada ATM")
metric_card(cols_cards[6], "total",     "🌙", int(df["is_247"].sum()) if not df.empty else 0,           "Buka 24/7")
metric_card(cols_cards[7], "total",     "⚡", int(df["has_ev"].sum()) if not df.empty else 0,           "EV Charging")
...
with map_col:
    if not user_loc_active:
        st.markdown("""
        <div style="background:rgba(168,85,247,.07);border:1px solid rgba(168,85,247,.2);
                    border-radius:10px;padding:8px 14px;margin-bottom:8px;
                    font-size:12px;color:#8A8FA8;display:flex;align-items:center;gap:8px">
            <span style="font-size:16px">🖱</span>
            <span><b style="color:#C084FC">Klik pada peta</b> untuk menentukan lokasi Anda — SPBU terdekat akan otomatis dihitung.</span>
        </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown(f"""
        <div style="background:rgba(168,85,247,.1);border:1px solid rgba(168,85,247,.3);
                    border-radius:10px;padding:8px 14px;margin-bottom:8px;
                    font-size:12px;color:#C084FC;display:flex;align-items:center;gap:8px">
            <span style="font-size:16px">📍</span>
            <span>Lokasi aktif: <b>{user_lat:.5f}, {user_lng:.5f}</b> — Klik lokasi lain untuk memperbarui.</span>
        </div>
        """, unsafe_allow_html=True)
    ...
    map_center = [user_lat, user_lng] if user_loc_active else [-6.2349, 106.9945]
    map_zoom   = 13 if user_loc_active else 12
    ...
    # ── LAYER 1: Polygon Batas Kota Bekasi ─────────────────────────────────
    if show_polygon and polygon_features:
        poly_layer = folium.FeatureGroup(name="🏙 Batas Kota Bekasi", show=True)
        for feat in polygon_features:
            poly_name = feat["properties"].get("name", "Kota Bekasi")
            folium.GeoJson(
                feat,
                ...
                tooltip=folium.Tooltip(
                    f"<b style='font-family:DM Sans,sans-serif;color:#C084FC'>🏙 {poly_name}</b>"
                    f"<br><span style='font-size:11px;color:#8A8FA8'>Batas Administrasi Kota Bekasi</span>",
                    sticky=False,
                ),
            ).add_to(poly_layer)
        poly_layer.add_to(m)

    # ── LAYER 2: Road / LineString ──────────────────────────────────────────
    if show_roads and road_features:
        road_layer = folium.FeatureGroup(name="🛣 Jaringan Jalan", show=True)
        for feat in road_features:
            hw = feat["properties"].get("highway", "unclassified")
            if hw in ROAD_SKIP_TYPES or hw not in ROAD_COLORS:
                continue
            road_color  = ROAD_COLORS.get(hw, "#2E3245")
            road_weight = ROAD_WEIGHTS.get(hw, 0.8)
            road_name   = feat["properties"].get("name", "Jalan tanpa nama")
            folium.GeoJson(
                feat,
                style_function=lambda x, c=road_color, w=road_weight: {
                    "color":   c,
                    "weight":  w,
                    "opacity": 0.75,
                },
                tooltip=folium.Tooltip(
                    ...
                    f"<b style='color:#F0F0F5'>{road_name}</b><br>"
                    f"<span style='color:#5A5F72'>{hw.replace('_', ' ').title()}</span>"
                    f"</span>",
                    sticky=False,
                ),
            ).add_to(road_layer)
    ...
    if user_loc_active:
        folium.Marker(
            ...
            tooltip="📍 Lokasi Anda",
            popup=folium.Popup(
                f"<b>Lokasi Anda</b><br>{user_lat:.6f}, {user_lng:.6f}",
                max_width=200,
            ),
    ...
    nearby_legend_item = (
        ...
        "<div style='width:12px;height:12px;border-radius:50%;border:2px solid #A855F7;flex-shrink:0'></div>"
        "<span style='font-size:11px'>SPBU Terdekat</span></div>"
    ) if user_loc_active else ""

    road_legend = ""
    if show_roads:
        road_legend = """
        <div style="border-top:1px solid rgba(255,255,255,.08);margin-top:8px;padding-top:6px">
          <div style="font-size:10px;text-transform:uppercase;letter-spacing:.08em;color:#5A5F72;margin-bottom:6px">🛣 Jalan</div>
          <div style="display:flex;align-items:center;gap:6px;margin:3px 0"><div style="width:20px;height:3px;background:#00F5FF;border-radius:2px;box-shadow:0 0 4px #00F5FF"></div><span style="font-size:10px">Tol / Arteri</span></div>
          <div style="display:flex;align-items:center;gap:6px;margin:3px 0"><div style="width:20px;height:3px;background:#0096FF;border-radius:2px;box-shadow:0 0 4px #0096FF"></div><span style="font-size:10px">Jalan Primer</span></div>
          <div style="display:flex;align-items:center;gap:6px;margin:3px 0"><div style="width:20px;height:2px;background:#0060CC;border-radius:2px"></div><span style="font-size:10px">Jalan Sekunder</span></div>
        </div>
        """
    poly_legend = ""
    if show_polygon:
        ...
        <div style="margin-top:4px;display:flex;align-items:center;gap:6px">
          <div style="width:12px;height:12px;border:2px dashed #C084FC;border-radius:2px;flex-shrink:0"></div>
          <span style="font-size:11px">Batas Kota Bekasi</span>
        </div>
        """
    ...
    <div style="position:fixed;bottom:30px;left:20px;z-index:9999;
                ...
                box-shadow:0 4px 20px rgba(0,0,0,.5);max-width:200px">
    ...
    map_data = st_folium(
        ...
        key="bekasi_map" # Menambahkan key eksplisit untuk kestabilan widget
    )

    # 🛑 CRITICAL FIX: Mencegah Loop Penyebab Crash
    if map_data and map_data.get("last_clicked"):
        ...
        if new_lat is not None and new_lng is not None:
            # Gunakan math.isclose untuk mengatasi ketidakpastian nilai float koordinat
            old_lat = st.session_state.user_lat
            old_lng = st.session_state.user_lng
            
            is_changed = (old_lat is None or old_lng is None or 
                          not math.isclose(new_lat, old_lat, abs_tol=1e-6) or 
                          not math.isclose(new_lng, old_lng, abs_tol=1e-6))
            
            if is_changed:
                st.session_state.user_lat = new_lat
                st.session_state.user_lng = new_lng
                st.rerun()
...
with detail_col:
    if user_loc_active:
        n_in_radius = len(df_within_radius)
        st.markdown(f"""
        ...
        if len(df_nearest) > 0:
            for rank, (_, r) in enumerate(df_nearest.iterrows(), 1):
                ...
                badges_near_html = (
                    "<div style='margin-top:5px;font-size:10px;color:#6B9FE4'>" +
                    " · ".join(badges_near) +
                    "</div>"
                ) if badges_near else ""
                st.markdown(f"""
    ...
    fa_col1.metric("🏧 ATM", int(df["atm"].sum()) if not df.empty else 0)
    fa_col2.metric("🚻 Toilet", int(df["toilets"].sum()) if not df.empty else 0)
    fa_col3, fa_col4 = st.columns(2)
    fa_col3.metric("🌙 24/7", int(df["is_247"].sum()) if not df.empty else 0)
    fa_col4.metric("⚡ EV", int(df["has_ev"].sum()) if not df.empty else 0)
...
if not df.empty:
    ...
    with ch4:
        ...
        fuel_counts = {}
        # Menghitung ketersediaan BBM secara dinamis dari DataFrame terfilter agar sinkron
        for key, label in fuel_map_display.items():
            count = 0
            for _, r in df.iterrows():
                if label in r["fuels"] or (key == "fuel:electricity" and r["has_ev"]):
                    count += 1
            if count > 0:
